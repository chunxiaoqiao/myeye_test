...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        for(int i=0;i<points_data.frame.rows;i++)
        {
            ...
            {
                PointT p ;
                if(1){//depth_data.frame.at<uint16_t>(i, j)/100>0) {
                    p.x =points_data.frame.at<cv::Vec3b>(i,j)[0];
                    p.y =points_data.frame.at<cv::Vec3b>(i,j)[1];
                    p.z =points_data.frame.at<cv::Vec3b>(i,j)[2];
                    p.b = img.at<cv::Vec3b>(i, j)[2];
                    p.g = img.at<cv::Vec3b>(i, j)[1];
                    p.r = img.at<cv::Vec3b>(i, j)[0];
...
//        if (viewer_ == nullptr) {   //实时显示
//            viewer_ = simpleVis(pointCloud2);
//        }
//        viewer_->updatePointCloud(pointCloud2, "3D Viewer");
//        viewer_->spinOnce();

        pcl::visualization::CloudViewer viewer ("Viewer_test");
        viewer.showCloud (pointCloud2);
        while (!viewer.wasStopped ())
        {
        }
        pcl::io::savePCDFileBinary("../map_qiao.pcd",*pointCloud2  );
        /////////////////
