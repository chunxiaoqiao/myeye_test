...
int main(int argc, char *argv[]) {
  ...
  bool ok;
  auto &&request = api->SelectStreamRequest(&ok);//输出相机选项，等待输入选项
  if (!ok) return 1;
  api->ConfigStreamRequest(request);

  // Enable motion datas with timestamp correspondence of some stream
  api->EnableTimestampCorrespondence(Stream::LEFT);//让获取到的图像的时间戳，在获取到的 IMU 数据的时间戳中间，保持对应关系
  ...
  while (true) {
    ...
    if (!left_data.frame.empty() || !right_data.frame.empty()) {
      ...
      for (size_t i = 0, n = motion_datas.size() - 1; i <= n; ++i) {
        ...
        std::cout<<"accell="<<data.imu->accel[0]<<"  "<<data.imu->accel[1]<<"  "<<data.imu->accel[2]<<std::endl;//测试_乔

        std::stringstream ss;
        if (imu_flag == 0) {  // accel + gyro
          ss << "Imu";
        } else if (imu_flag == 1) {  // accel 加速度
          ss << "Accel";
        } else if (imu_flag == 2) {  // gyro陀螺仪
          ss << "Gyro";
        }
